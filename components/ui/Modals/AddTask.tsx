...
setAddTaskForm((prevState)=>({...prevState,description:e.target.value}))
}
...
const res= await fetch('/api/tasks/create', {
      ...
      body: JSON.stringify({description:addTaskForm.description,pocketId:addTaskForm.pocketId}),
    });
    if(!res.ok){
...
<div className={`${addTaskForm.errorDescription?'border-[1px] border-red-600':'border-0'} flex flex-row w-full h-[44px] py-2.5 px-3 bg-gray-50 gap-3 rounded-[10px] items-center`}>
<span className='w-[24px] h-[24px] rounded-[6px] bg-white block shrink-0 border-[1px] border-gray-200'></span>
<input onChange={descriptionHandler} type='text' placeholder='Create a new task' className='w-full stroke-0 outline-none text-[14px] font-normal'/>
<button type='submit' className='flex items-center justify-center py-[10px] px-[12px] bg-gray-100 rounded-[8px] text-[14px] font-semibold cursor-pointer'>Create</button>
</div>
<div className='flex flex-col gap-2'>
<span className='px-1.5 text-[14px] font-medium'>Select pocket</span>
<div className={`${addTaskForm.errorPocket?'border-[1px] border-red-600':'border-0'} flex flex-col gap-1.5`}>
{pockets.map((pocket)=><InputSelectPocket {...pocket} selected={addTaskForm.pocketId} onChange={selectPocketHandler} key={pocket._id}/>)}
<ButtonCreateNewPocket onClick={showNewPocket}/>
</div>
        </div>
            </form>
