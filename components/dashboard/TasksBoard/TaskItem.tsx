...
const deleteTaskHandler = async() =>{
 try {
    ...
    return <div className='flex flex-col gap-1.5 items-end'><div className={`flex flex-row justify-between py-[4px] pl-[8px] pr-[4px] w-full ${isCompleted?"bg-[#6529FE]":'bg-white'} rounded-[6px]`}>
...
<button onClick={showDeleteButtonHandler} className={`flex items-center justify-center ${isCompleted?'bg-[#754EFF]':'bg-gray-50'} rounded-[4px] w-[30px] h-[30px] cursor-pointer`}>
    <img src={isCompleted?DotsWhiteIcon.src:DotsIcon.src} className='w-[14px] h-[14px]' alt='dots icon'/>
</button>

    </div>
    {showDeleteButton&&<button onClick={deleteTaskHandler} className='h-min w-fit items-center py-2.5 px-3 flex flex-row gap-[10px] bg-white rounded-[6px] drop-shadow-black cursor-pointer'>
    <img src={TrashIcon.src} className='w-[14px] h-[14px]' alt='dots icon'/> <span className='text-[14px] text-[#C54D4D] font-normal'>Delete</span>
</button>}
    </div>
}