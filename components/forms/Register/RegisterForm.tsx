...
setRegisterForm((prevState:RegisterFormType)=>({...prevState,login:{...prevState.login,value:e.target.value}}))
}
const passwordHandler = (e:React.ChangeEvent<HTMLInputElement>) =>{
setRegisterForm((prevState:RegisterFormType)=>({...prevState,password:{...prevState.password,value:e.target.value}}))
...
onSuccess(registerForm.login.value,registerForm.password.value)
    ...
    <div className='flex flex-col gap-8 w-full sm:w-[520px] shrink-0 py-10 px-5 sm:py-44 sm:px-14  '>
      ...
      <form className='flex flex-col gap-2.5' onSubmit={submitFormHandler}>
       <InputTextForm value={registerForm.login.value} icon={UserIcon} onChange={loginHandler} success={registerForm.login.success} error={registerForm.login.error} placeholder='Username'/>
       <InputTextForm value={registerForm.password.value} icon={PasswordIcon} onChange={passwordHandler} success={registerForm.password.success} error={registerForm.password.error} placeholder='Password'/>
        
<button type='submit' className='py-2.5 px-4 bg-purple-600 text-white rounded-lg w-fit h-min'>Login me</button>
     </form>
    <p className='font-normal'>Have account already? <Link className='text-purple-600' href="/login">Login</Link></p>
      {fetchError && (
