...
if (!result.success) {
 ...
 const res= await fetch('http://localhost:3000/api/auth/login', {
    ...
    if(!res.ok){
     if(res.status === 401){
      setLoginForm((prevState)=>({...prevState,success:false,error:"Wrong login or password"}))
     }else {
      setLoginForm(prev => ({
        ...prev,
        success: false,
...
router.push('/dashboard')
    ...
    return <div className='flex flex-col gap-8 w-full sm:w-[520px] shrink-0 py-10 px-5 sm:py-44 sm:px-14  '>
      ...
      <form onSubmit={submitFormHandler} className='flex flex-col gap-2.5'>
        <InputTextForm value={loginForm.login.value} icon={UserIcon} onChange={loginHandler} success={loginForm.login.success} error={loginForm.login.error} placeholder='Username'/>
       <InputTextForm value={loginForm.password.value} icon={PasswordIcon} onChange={passwordHandler} success={loginForm.password.success} error={loginForm.password.error} placeholder='Password'/>
